package cn.edu.gdmec.s07150745.viewflipperdemo;
...
public class MainActivity extends AppCompatActivity implements GestureDetector.OnGestureListener{
    ...
    private static final int FILNG_MIN_DISTANCE = 100;
    private static final int FLING_MIN_VELOCIIY = 200;
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
       if (e2.getX()-e1.getX()>FILNG_MIN_DISTANCE && Math.abs(velocityX)>FLING_MIN_VELOCIIY){
           viewFlipper.setInAnimation(lInAnim);
           viewFlipper.setOutAnimation(rOutAnim);
           viewFlipper.showPrevious();
           setTitle("相片编号："+viewFlipper.getDisplayedChild());
           return true;
       }else if (e1.getX()-e2.getX()>FILNG_MIN_DISTANCE&&Math.abs(velocityX)>FLING_MIN_VELOCIIY){
